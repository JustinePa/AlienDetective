...
#land_polygons <- st_transform(land_polygons, crs = 4326)
...
input_species_file <- args[1]  # First argument: path to species file
input_coordinates_file <- args[2]  # Second argument: path to coordinates file
output_directory <- args[3]  # Third argument: output directory
...
if (!dir.exists("OccurrenceData")) {
  dir.create("OccurrenceData", recursive = TRUE)
}
...
output_sea_dir <- file.path(output_directory, "sea_distances")
output_fly_dir <- file.path(output_directory, "fly_distances")

if (!dir.exists(output_sea_dir)) {
  dir.create(output_sea_dir, recursive = TRUE)
}

if (!dir.exists(output_fly_dir)) {
  dir.create(output_fly_dir, recursive = TRUE)
}
...
foreach(species_name = unique(long$Specieslist),
        .packages = c("rgbif"),
        .export = c("ensure_columns", "fetch_data_in_batches")) %dopar% {
...
#   ##########################################################################
# 
#   # Load and rasterize the custom shapefile
#   r <- raster(extent(-180, 180, -90, 90), res = 0.1)
#   r <- rasterize(land_polygons, r, field = 1, fun = max, na.rm = TRUE)
...
#   
#   # Define file paths
#   sea_distance_file <- paste0("Output_calculations/sea_distances/", species_name, "distancesTo", species_location, ".csv")
#   fly_distance_file <- paste0("Output_calculations/fly_distances/", species_name, "distancesTo", species_location, ".csv")
#   
